...
def resize_and_copy_image(src_path, dest_path, target_size=(256, 256)):
    """
    Open an image from src_path, resize it while preserving aspect ratio using
    the LANCZOS filter, and paste it onto a transparent canvas of target_size.
    ...
    # Convert image to RGBA to ensure transparency support
    if img.mode != "RGBA":
...
def process_logos(source_folder, dest_folder, target_size=(256, 256)):
    """
    Process all logo files from source_folder and save resized copies with transparent 
    backgrounds into dest_folder. For SVG files, convert them to PNG.
    """
    os.makedirs(dest_folder, exist_ok=True)
    # Add '.webp' to the list of supported raster formats
    ...
    for filename in os.listdir(source_folder):
        ...
        if ext in supported_raster:
            # Save output as PNG to support transparent backgrounds
